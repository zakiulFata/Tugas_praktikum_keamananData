...
root.configure(bg="#e0f7fa")  # Background utama lebih cerah

# Gaya global untuk widget
style = ttk.Style()
style.configure("TLabel", font=("Helvetica", 12), background="#e0f7fa", foreground="#333")
style.configure("TButton", font=("Helvetica", 10, "bold"), foreground="#000000", background="#00796b", padding=5)  # Mengubah warna teks tombol menjadi hitam
style.map("TButton", background=[('active', '#004d40')])  # Warna saat hover
...
# Judul aplikasi
title_label = ttk.Label(main_frame, text="Program Enkripsi dan Deskripsi Text", font=("Helvetica", 23, "bold"), foreground="#007acc")
...
button_encrypt = ttk.Button(button_frame, text="Enkripsi", command=enkripsi_text, style="TButton")
button_encrypt.pack(side=tk.LEFT, padx=10)

button_decrypt = ttk.Button(button_frame, text="Deskripsi", command=deskripsi_text, style="TButton")
button_decrypt.pack(side=tk.LEFT, padx=10)

# Footer
footer_label = ttk.Label(root, text="Dibuat oleh Zakiul Fata & GPT", font=("Helvetica", 10), foreground="#000000")  # Mengubah warna menjadi hitam
